...
abstract class Builder extends Query
{
    ...
    protected function bulidSelect()
    {

        $sql = str_replace(
            array('%TABLE%', '%DISTINCT%', '%FIELD%', '%JOIN%', '%WHERE%', '%GROUP%', '%HAVING%', '%ORDER%', '%LIMIT%', '%UNION%', '%LOCK%', '%COMMENT%', '%FORCE%'),
            array($options['table'], $options['distinct'], $options['field'], $options['join'], $options['where'], $options['group'], $options['having'], $options['order'], $options['limit'], $options['union'], $options['lock'], $options['comment'], $options['force'],
            ), $this->selectSql);
    ...
    protected function parseData($data, $options)
    {
        ...
        // 获取绑定信息
        $bind = parent::getFieldsBind($options['table']);
        if ('*' == $options['field']) {
        ...
        foreach ($data as $key => $val) {
            ...
            if (false === strpos($key, '.') && !in_array($key, $fields, true)) {
                ...
                // 过滤非标量数据
                if (0 === strpos($val, ':') && parent::isBind(substr($val, 1))) {
                    $result[$item] = $val;
                } else {
                    $key = str_replace('.', '_', $key);
                    parent::bind('data__' . $key, $val, isset($bind[$key]) ? $bind[$key] : PDO::PARAM_STR);
    ...
    {
        if (is_string($value)) {
            $value = strpos($value, ':') === 0 && parent::isBind(substr($value, 1)) ? $value : $this->connection->quote($value);
     ...
     * @return string
     */
    protected function parseTable($tables, $isTablepre = true)
    {
        ...
        foreach ((array) $tables as $key => $table) {
            $item[] = $prefix != '' && $isTablepre ? $prefix . $table : $table;
        }
    ...
    /** 多条件查询 */
    protected function parseWhereMap($where)
    {

        foreach ((array) $where as $key => $value) {
            $whereStr .= $this->parseWhere($key, $value, $condition);
        }
     ...
     * @return string
     */
    protected function parseWhere($key, $value)
    {

        $whereStr = $this->parseWhereKey($key, $value);

        $this->options['whereData'][] = array(
            'value'     => $whereStr,
            'condition' => 'AND',
        );

        $whereStr = $this->buildWhere();
    ...
    public function buildWhere()
    {
        ...
        $whereStr = '';
        if (!$this->options['whereData']) {
            return '';
        }
        foreach ($this->options['whereData'] as $key => $value) {
            $whereStr .= $value['value'] . ' ' . $value['condition'] . ' ';
        }
    ...
    /** 分析where key */
    protected function parseWhereKey($key, $value)
    {
        ...
        if ($length !== false) {
            ...
            $field        = substr($key, 0, $length);
            $keyCondition = strtoupper(trim(substr($key, $length, strlen($key))));

            $whereStr = $this->parseWhereKeyCondition($field, $value, $keyCondition);

        } elseif (strtoupper($key) == '_STRING') {
            $whereStr = $value;
        } else {
            $whereStr = $key . ' = ' . $value;
        }
    ...
    protected function parseWhereKeyCondition($key, $value, $condition)
    {
        ...
        switch ($condition) {
            case 'IN':
            case 'NOT IN':
                $whereStr = '`' . $key . '` ' . $condition . '(' . $valueStr . ') ';
                break;
            case '>':
            case '<':
            case '=':
            ...
            case 'NOT BETWEEN':
            case 'BETWEEN':
                $whereStr = '`' . $key . '` ' . $condition . ' \'' . $value[0] . '\' AND \'' . $value[1] . '\' ';
                break;
            default:
                # code...
                break;
    ...
    protected function parseWhereItem($field, $val, $rule = '', $options = [], $binds = [], $bindName = null)
    {
        ...
        if (is_scalar($value) && array_key_exists($field, $binds) && !in_array($exp, ['EXP', 'NOT NULL', 'NULL', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN']) && strpos($exp, 'TIME') === false) {
            if (strpos($value, ':') !== 0 || !parent::isBind(substr($value, 1))) {
                if (parent::isBind($bindName)) {
                    $bindName .= '_' . str_replace('.', '_', uniqid('', true));
                }
                parent::bind($bindName, $value, $bindType);
                $value = ':' . $bindName;
            }
        }
        ...
        if (in_array($exp, ['=', '<>', '>', '>=', '<', '<='])) {
            ...
            if ($value instanceof \Closure) {
                ...
                if (array_key_exists($field, $binds)) {
                    ...
                    foreach ($value as $v) {
                        $i++;
                        if (parent::isBind($bindName . '_in_' . $i)) {
            ...
            if (array_key_exists($field, $binds)) {
                if (parent::isBind($bindName . '_between_1')) {
    ...
    protected function parseDateTime($value, $key, $options = [], $bindName = null, $bindType = null)
    {
        ...
        $type = parent::getTableInfo($table, 'type');
        if (isset($type[$key])) {
            $info = $type[$key];
        }
        if (isset($info)) {
    ...
    public function insertAll($dataSet, $options = [], $replace = false)
    {
        // 获取合法的字段
        if ('*' == $options['field']) {
            $fields = array_keys(parent::getFieldsType($options['table']));
        } else {
            $fields = $options['field'];
        }
